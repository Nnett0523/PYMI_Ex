...
def solve(input_data):
    ...
    provinces_startswith_h = [(dat['name'], dat['population']) \
                            for dat in input_data if dat['name'][0] == 'H']
    provinces_startswith_h = sorted(provinces_startswith_h, \
                                key=lambda province: province[0])

    provinces_more_than_1m = [(dat['name'], dat['population']) \
                            for dat in input_data \
                            if dat['population'] > 1000000]
    provinces_more_than_1m = sorted(provinces_more_than_1m, \
                                key=lambda province:province[1], reverse=True)
