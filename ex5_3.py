...
def solve(input_data):
    ...
    new_data = ''
    char_allow = string.ascii_letters + '’' + ' '
    for dat in input_data:
        if dat in char_allow:
            new_data += dat

    arr_checked = []
    result = []
    for dat in new_data.split():
        if dat not in arr_checked:
            result.append((dat, new_data.split().count(dat)))
            arr_checked.append(dat)
    sorted(result, key=lambda result: result[1], reverse=True)
    result = sorted(result, key=lambda result: result[1], reverse=True)[:5]
    # arr_not_same = []
    # last_result = []
    # for ele in result:
    #     if ele[1] not in arr_not_same:
    #         arr_not_same.append(ele[1])
    #         last_result.append(ele)
